    ...
    getDataFromLocal().then((value) {
      if(isLogin!){
        Timer(const Duration(seconds: 3), () {
          Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (context) =>   const LoginScreenFixed(),), (route) => false);
        });
      } else{
        Timer(const Duration(seconds: 3), () {
      ...
      body: Center(
        child: Column(
          ...
          children: [
            ...
            Text('kkc_reservation',style: TextStyle(color: Color(0xff7D2AFF),fontSize: 25,fontFamily: 'SofiaProBold'),),
          ],
        ),
      ),
    );
